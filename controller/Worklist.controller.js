sap.ui.define(["./BaseController",
	...
	return BaseController.extend("zmm003.zsrvreq.controller.Worklist", {
		...
		_onCreateSerAgr: function() {

			this.getRouter().navTo("SerAgrRequest", {
				ReqNo: '0000000000',
				Approver : false
			});
			...
			this.getRouter()._oViews._oViews["zmm003.zsrvreq.view.SerAgrRequest"].setModel(oCrtSerAgr, "oCrtSerAgr");
		...
		onInit: function() {
			...
			this.setModel(new JSONModel(oSerAgrData), 'SerAgr');
		...
		onExportData: function(oEvent) {
			...
			var oExport = new Export({
				...
				columns: [{
					"name": oResourceBundle.getText("ID Number"),
					"template": {
						"content": "{ReqNo}"
					}
				}, {
					"name": oResourceBundle.getText("Solicitation Number"),
					"template": {
						"content": "{Solicitation}"
					}
				}, {
					"name": oResourceBundle.getText("Date"),
					"template": {
						"content": "{OnDate}"
					}
				}, {
					...
					"name": oResourceBundle.getText("Firm Name"),
					"template": {
						"content": "{FirmName}"
					}
				}, {
					...
					"name": oResourceBundle.getText("Value"),
					"template": {
						"content": "{path : 'Value', type: 'sap.ui.model.type.Float', formatOptions:{ minFractionDigits: 2,groupingSeparator: ',',decimalSeparator: '.', maxFractionDigits:2 } }"
					}
				}, {
					...
					"name": oResourceBundle.getText("Approved By"),
					"template": {
		...
		onUpdateFinished: function(oEvent) {
			...
			if (iTotalItems && oTable.getBinding("items").isLengthFinal()) {
				...
				oFilter = [];
				oFilter[0] = new sap.ui.model.Filter("ReqTy", sap.ui.model.FilterOperator.EQ, 'SAR');
				...
				oFilter[1] = new sap.ui.model.Filter("ReqTy", sap.ui.model.FilterOperator.EQ, 'SAR');
		...
		onSearch: function(oEvent) {
			var oFilters = this.getModel('SerAgr').getProperty("/filters");
			var aFilter = [];
			...
			//Get Table Binding
			var oBinding = this.getView().byId('idSerAgrTbl').getBinding("items");
			oBinding.filter(aFilter);
		},
		handleF4RequestorWV: function(oEvent) {
			if (!this.onf4Requestorhelpdialog) {
				this.onf4Requestorhelpdialog = sap.ui.xmlfragment("zmm003.zsrvreq.fragment.onF4RequestorWV", this);
		...
		onReqVHConfirmWV: function(oEvent) {
			var oInput = this.byId("idSerAgrRequestorWV");
